...
def main() -> None:
    ...
    retirement_age = st.sidebar.number_input("退職年齢", min_value=min_ret_age, max_value=75, value=default_ret_age, step=1, key="retirement_age")
    pension_start_age = st.sidebar.number_input("年金受給開始年齢", min_value=60, max_value=75, value=65, step=1, key="pension_start_age")
    ...
    life_expectancy = st.sidebar.number_input("想定寿命", min_value=min_life, max_value=120, value=min(default_life, 120), step=1, key="life_expectancy")
    st.sidebar.markdown("---")
    ...
    st.sidebar.markdown("---")
    base_income_man = st.sidebar.number_input("現在の年収（万円）", min_value=200, max_value=2000, value=460, step=10, key="base_income_man")
    growth_rate_pct = st.sidebar.slider("年収成長率（%）", min_value=0.0, max_value=10.0, value=3.0, step=0.1, key="growth_rate_pct")
    ...
    with st.sidebar.expander("🛠️ 開発者情報", expanded=False):
        ...
        st.caption("この表示で、バックアップ側のモジュールが誤って読み込まれていないか確認できます。")

    # タブ構成
    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["🏠 ホーム", "💰 支払実績", "🎯 受給額試算", "🔮 将来予測", "📊 損益分岐・最適化", "📋 計算方法"])

    with tab1:
        st.subheader("概要")
        st.info("この画面は 高度版（損益分岐・最適化 含む）です。サイドバー下部の『🛠️ 開発者情報』でロード元パスを確認できます。", icon="✅")
    ...
    with tab2:
            ...
            st.caption("表は編集できます。CSV/Excelからの取り込みやCSVダウンロードにも対応しています。保存するとCSVに書き出され、以後はその内容がSoTになります。")
            with st.expander("📥 実績ファイルのインポート（CSV/Excel）"):
                up = st.file_uploader("ファイルを選択", type=["csv", "xlsx", "xls"], accept_multiple_files=False, key="records_upload")
                if up is not None:
                    try:
            ...
            edited = st.data_editor(
                ...
                column_config={
                    "年度": st.column_config.NumberColumn("年度", min_value=1900, max_value=2100, step=1),
                    ...
                    "加入月数": st.column_config.NumberColumn("加入月数", min_value=0, max_value=12, step=1),
                    "納付額": st.column_config.NumberColumn("納付額", min_value=0, step=1000, format=",d"),
                    "推定年収": st.column_config.NumberColumn("推定年収", min_value=0, step=10000, format=",d"),
                },
            )
            csv_bytes = putils.df.to_csv(index=False).encode("utf-8-sig")
            st.download_button("📤 現在データをCSVでダウンロード", data=csv_bytes, file_name="pension_records.csv", mime="text/csv")
            ...
            fig_income.add_trace(go.Scatter(x=edited["年度"], y=edited["推定年収"], mode="lines+markers", name="年収", line=dict(color="blue")))
            fig_income.update_layout(title="年収（実績）", xaxis_title="年度", yaxis_title="年収（円）", height=360)
            st.plotly_chart(fig_income, config={"responsive": True})

            st.markdown("### 💰 納付額推移（実績）")
            fig_paid = go.Figure()
            fig_paid.add_trace(go.Bar(x=edited["年度"], y=edited["納付額"], name="納付額", marker_color="green"))
            fig_paid.update_layout(title="年間納付額（実績）", xaxis_title="年度", yaxis_title="金額（円）", height=360)
    ...
    with tab3:
        ...
        with c3:
            st.metric("合計（年額）", f"{total_annual:,.0f} 円", delta=f"月額 {total_annual/12:,.0f} 円")
        ...
        fig.update_layout(title="年間受給額（万円）", xaxis_title="年齢", yaxis_title="金額（万円）", height=380)
        st.plotly_chart(fig, config={"responsive": True})

    with tab4:
        st.subheader("🔮 将来予測")
        remaining_years = max(0, int(retirement_age) - int(current_age))
        incomes = putils.estimate_income_by_company_growth(base_income_yen, growth_rate, remaining_years)
        ...
        if remaining_years > 0 and len(incomes) == remaining_years:
            fig_f = go.Figure()
            fig_f.add_trace(go.Scatter(x=years_axis, y=incomes, mode="lines+markers", name="推定年収"))
            fig_f.update_layout(title="推定年収（円）", xaxis_title="年度", yaxis_title="年収（円）", height=360)
            st.plotly_chart(fig_f, config={"responsive": True})
        else:
            st.info("将来年数が0年のため、年収予測は表示できません。")

        st.markdown("### 🏛️ 国民年金保険料（実績・予測）")
        years_actual, national_history, future_years_pension, future_fees = putils.generate_national_pension_projection()
        fig_p = go.Figure()
        fig_p.add_trace(go.Scatter(x=years_actual, y=national_history, mode="lines+markers", name="実績"))
        fig_p.add_trace(go.Scatter(x=future_years_pension, y=future_fees, mode="lines+markers", name="予測", line=dict(dash="dash")))
        fig_p.update_layout(title="国民年金月額保険料", xaxis_title="年度", yaxis_title="月額（円）", height=360)
        st.plotly_chart(fig_p, config={"responsive": True})

        st.markdown("---")
        st.caption("注意: 本アプリは教育目的の簡易モデルです。実際の受給額は公式情報をご確認ください。")
    ...
    with tab5:
        ...
        def calculate_detailed_breakeven_analysis():
            ...
            for age in plot_ages:
                ...
                if not age_data.empty:
                    ...
                    # 将来推定納付額（より詳細）
                    if age >= 20:  # 20歳以降の納付
                        estimated_payment = min(avg_salary * 0.183, 500000)  # 厚生年金料率18.3%上限あり
        ...
        with col1:
            st.metric(
                ...
                delta=f"受給開始から{breakeven_age - int(pension_start_age)}年" if breakeven_age else "寿命内で回収不可"
            )
        ...
        with col3:
            ...
            if len(cumulative_data) > 0:
                ...
                if final_data['累計納付額'] > 0:
                    investment_return_rate = (final_data['累計受給額'] / final_data['累計納付額']) * 100
            
            st.metric(
                "投資回収率", 
                f"{investment_return_rate:.1f}%",
                delta="生涯受給額/生涯納付額"
            )
        ...
        if optimization_results:
            ...
            df_display["生涯総受給額"] = df_display["生涯総受給額"].map(lambda x: f"{x/1_000_000:.1f} 百万円")
