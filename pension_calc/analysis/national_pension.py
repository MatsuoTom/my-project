...
class NationalPensionAnalyzer:
    ...
    def calculate_lifetime_value(
        self, start_age: int = 20, retirement_age: int = 65, life_expectancy: int = 85
    ) -> Dict:
        """
        国民年金の生涯価値を計算
        ...
        monthly_pension = full_basic_pension_monthly * (
            actual_contribution_months / max_contribution_months
        )
        ...
        return {
            ...
            "投資利回り相当": roi,
            "損益分岐年数": (
                (total_contribution / annual_pension) if annual_pension > 0 else float("inf")
            ),
        }

    def optimize_receiving_age(self, start_age: int = 20, life_expectancy: int = 85) -> Dict:
        """
        最適な受給開始年齢を分析
    ...
    def analyze_fee_impact(self) -> Dict:
        ...
        # 将来予測
        years_actual, _, future_years, future_fees = generate_national_pension_projection(
            avg_growth_rate / 100
        )
