...
class TaxDeductionHelper:
    ...
    def calculate_annual_tax_savings(
        self, annual_premium: float, taxable_income: float = 5_000_000
    ) -> Dict[str, float]:
        """
        年間保険料から節税額を一括計算
    ...
    def calculate_total_tax_savings_over_years(
        self, annual_premium: float, years: int, taxable_income: float = 5_000_000
    ) -> float:
        """
        複数年の節税額合計を計算
    ...
    def compare_premium_scenarios(
        self, premium_options: list[float], taxable_income: float = 5_000_000
        ...
        for premium in premium_options:
            results[premium] = self.calculate_annual_tax_savings(premium, taxable_income)
        return results
