...
def main():
    ...
    st.markdown("---")
    st.markdown("**生命保険料控除を使った場合と、控除を使わずにeMAXIS Slim S&P500に投資した場合の実質的なリターンを比較します**")
...
def show_insurance_vs_investment_comparison():
    ...
    with col2:
        ...
        investment_fee = st.number_input(
            "信託報酬 (%)", 
            min_value=0.0, 
            max_value=1.0, 
            value=0.0968, 
        ...
        # 保険設定
        st.markdown("**🏦 生命保険設定**")
        insurance_return = st.number_input(
            "保険年利 (%)", 
            min_value=0.5, 
            max_value=3.0, 
            value=1.25, 
        ...
        setup_fee_rate = st.number_input(
            "初回手数料 (%)", 
            min_value=0.5, 
            max_value=3.0, 
            value=1.3, 
        ...
        monthly_fee_rate = st.number_input(
            "年間管理手数料 (%)", 
            min_value=0.005, 
            max_value=0.02, 
            value=0.096, 
    ...
    if st.button("🔍 詳細比較分析を実行", type="primary"):
        with st.spinner("計算中..."):
            ...
            insurance_scenario = calculate_insurance_investment_scenario(
                monthly_amount, annual_income, insurance_return, setup_fee_rate, 
                monthly_fee_rate, withdrawal_strategy, investment_return, investment_fee, 
                analysis_years, tax_calc, reinvest_strategy
            )
            
            # 結果表示
            display_comparison_results(
                investment_scenario, insurance_scenario, analysis_years, 
                monthly_amount, withdrawal_strategy
            )
...
def calculate_pure_investment_scenario(monthly_amount, annual_return, fee_rate, years):
    ...
    for year in range(1, years + 1):
        ...
        results.append({
            ...
            "annual_return": ((account_value / total_invested) ** (1/year) - 1) * 100 if total_invested > 0 else 0
        })
...
def calculate_insurance_investment_scenario(monthly_amount, annual_income, insurance_return, 
    ...
    if withdrawal_strategy == "元本回収後すぐに投資信託へ":
        withdrawal_year = calculate_breakeven_year(annual_premium, insurance_return, setup_fee_rate, monthly_fee_rate)
    ...
    for year in range(1, years + 1):
        ...
        results.append({
            ...
            "annual_return": ((total_value / total_invested) ** (1/year) - 1) * 100 if total_invested > 0 else 0,
...
def display_comparison_results(investment_scenario, insurance_scenario, years, monthly_amount, withdrawal_strategy):
    ...
    with tab1:
        comparison_df = pd.DataFrame({
            ...
            '差額': (df_insurance['total_value'] - df_investment['account_value']).apply(lambda x: f"{x:+,.0f}円"),
    ...
    with tab3:
        ...
        with col1:
            ...
            st.write(f"- 投資効率: {final_investment['profit']/final_investment['total_invested']*100:.1f}%")
        
        with col2:
            st.markdown("**保険＋投資信託**")
            st.write(f"- 最終年率リターン: {final_insurance['annual_return']:.2f}%")
            st.write(f"- 最大年間利益: {max([s['profit'] for s in insurance_scenario]):,.0f}円")
            st.write(f"- 投資効率: {final_insurance['profit']/final_insurance['total_invested']*100:.1f}%")
            st.write(f"- 節税効果: {total_tax_savings:,.0f}円 ({total_tax_savings/final_insurance['total_invested']*100:.1f}%)")
