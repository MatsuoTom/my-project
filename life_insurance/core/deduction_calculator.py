    ...
    def optimize_premium_distribution(
        ...
        def generate_distributions(remaining_budget, remaining_contracts, current_dist):
            ...
            if remaining_contracts == 1:
                ...
                total_deduction = sum(self.calculate_old_deduction(premium) 
                                    for premium in final_dist)
...
def main():
    ...
    for premium in test_premiums:
        ...
        print(f"保険料: {premium:,}円 → 控除額: {breakdown['控除額']:,}円 "
    ...
    print(f"最適: {result['最適な控除額']:,}円 ({'個別' if result['個別計算が有利'] else '合算'}計算)")
