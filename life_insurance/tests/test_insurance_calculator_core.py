...
class TestCalculatePartialWithdrawalValue:
    ...
    def test_nisa_reinvestment(self):
        ...
        fund_taxable = FundPlan(
            reinvestment_rate=5.0,
            use_nisa=False
        )
        
        result_nisa = calculator.calculate_partial_withdrawal_value(
            plan, 0.3, 5, fund_nisa
        )
        result_taxable = calculator.calculate_partial_withdrawal_value(
            plan, 0.3, 5, fund_taxable
        )
...
class TestInsuranceCalculatorIntegration:
    ...
    def test_realistic_scenario_comparison(self):
        ...
        # 2. 部分解約
        partial = calculator.calculate_partial_withdrawal_value(
            plan, 0.3, 5, fund, taxable_income
        )
        
        # 3. 乗り換え
        switching = calculator.calculate_switching_value(
            plan, 10, fund, taxable_income
        )
