...
import pytest
from life_insurance.utils.tax_helpers import TaxDeductionHelper, get_tax_helper, reset_tax_helper
...
class TestTaxDeductionHelper:
    ...
    def test_calculate_annual_tax_savings_different_premiums(self, helper):
        """異なる保険料での計算"""
        # 少額の保険料（第2段階: 25001～50000円）
        result_low = helper.calculate_annual_tax_savings(30000, 5000000)
        assert result_low["deduction"] == 20000  # 30000 * 0.25 + 12500 = 20000

        # 中額の保険料（第3段階: 50001～100000円）
        result_mid = helper.calculate_annual_tax_savings(60000, 5000000)
        assert result_mid["deduction"] == 27000  # 60000 * 0.2 + 15000 = 27000

        # 高額の保険料（第4段階: 100001円以上、上限50000円）
        result_high = helper.calculate_annual_tax_savings(150000, 5000000)
        assert result_high["deduction"] == 50000  # 第4段階では固定50000円
    ...
    def test_calculate_total_tax_savings_over_years(self, helper):
        ...
        total = helper.calculate_total_tax_savings_over_years(annual_premium, years, taxable_income)

        # 年間節税額の20倍
        annual_result = helper.calculate_annual_tax_savings(annual_premium, taxable_income)
    ...
    def test_calculate_total_tax_savings_over_years_single_year(self, helper):
        ...
        total = helper.calculate_total_tax_savings_over_years(annual_premium, 1, 5000000)

        annual = helper.calculate_annual_tax_savings(annual_premium, 5000000)
    ...
    def test_compare_premium_scenarios(self, helper):
        ...
        assert results[60000]["total_savings"] < results[100000]["total_savings"]
...
class TestTaxHelperEdgeCases:
    ...
    def test_very_low_income(self, helper):
        ...
        # 控除額（100000円は第3段階: 100000 * 0.2 + 15000 = 35000）
        assert result["deduction"] == 35000
...
class TestTaxHelperIntegration:
    ...
    def test_optimization_scenario(self):
        ...
        # 100,000円は第3段階（100000 * 0.2 + 15000 = 35000）
        assert results[100000]["deduction"] == 35000

        # 150,000円は第4段階で上限50,000円に到達
        assert results[150000]["deduction"] == 50000
