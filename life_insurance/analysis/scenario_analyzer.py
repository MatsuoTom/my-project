...
class ScenarioAnalyzer:
    ...
    def create_comprehensive_scenario(
        ...
        variation_params: Dict[str, List[Any]]
    ) -> pd.DataFrame:
        """
        包括的なシナリオ分析を実行
    ...
    def _run_single_scenario(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "投資効率": result["純利益"] / (annual_premium * policy_years) if (annual_premium * policy_years) > 0 else 0
        }
        ...
        for i, metric in enumerate(y_metrics):
            ...
            if group_by and group_by in scenario_results.columns:
                ...
                for group_value in scenario_results[group_by].unique():
                    ...
                    ax.plot(data[x_param], data[metric], 
                           marker='o', label=f"{group_by}={group_value}")
                ax.legend()
            else:
                # 単純プロット
                ax.plot(scenario_results[x_param], scenario_results[metric], 
    ...
    def generate_recommendation_report(
        ...
        return {
            ...
            "推奨アクション": self._generate_recommendations(best_scenario, risk_metrics, sensitive_params)
        }
    ...
    def _generate_recommendations(
        ...
        if risk_metrics:
            if risk_metrics.get("純利益_負の確率", 0) > 0.2:
                recommendations.append("純利益が負になる確率が20%を超えています。リスク許容度を再検討してください。")
            
            var_5 = risk_metrics.get("純利益_VaR_5%")
            if var_5 and var_5 < -100000:
                recommendations.append(f"最悪ケース（5%確率）で{abs(var_5):,.0f}円の損失リスクがあります。")
        
        # 感度ベース推奨
        for param in sensitive_params:
            if param["パラメータ"] == "return_rate":
                recommendations.append("運用利回りが結果に大きく影響します。保守的な想定を検討してください。")
            elif param["パラメータ"] == "taxable_income":
                recommendations.append("所得水準の変動が大きく影響します。将来の所得見通しを慎重に検討してください。")
...
def main():
    ...
    print("純利益上位5シナリオ:")
    print(top_scenarios[["シナリオID", "パラメータ_annual_premium", "パラメータ_taxable_income", 
                        "パラメータ_return_rate", "純利益", "実質利回り"]].to_string(index=False))
    
    print("\n2. 感度分析:")
    sensitivity_result = analyzer.analyze_sensitivity(
        base_params, "annual_premium", 
