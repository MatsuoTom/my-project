...
class TestFinancialPlanBasic:
    ...
    def test_create_basic_plan(self):
        """基本的なプラン作成のテスト"""
        plan = FinancialPlan(name="定期保険", start_age=30, end_age=60, annual_payment=100000)
    ...
    def test_create_lifetime_plan(self):
        """終身プラン作成のテスト"""
        plan = FinancialPlan(name="終身保険", start_age=30, end_age=None, annual_payment=150000)
    ...
    def test_create_with_default_payment(self):
        ...
        plan = FinancialPlan(name="無料相談", start_age=20, end_age=25)

        assert plan.annual_payment == 0.0

    def test_create_with_zero_payment(self):
        """支払額0円での明示的なプラン作成"""
        plan = FinancialPlan(name="無料プラン", start_age=20, end_age=25, annual_payment=0.0)
...
class TestFinancialPlanValidation:
    ...
    def test_negative_start_age_raises_error(self):
        ...
        with pytest.raises(ValueError, match="開始年齢は0以上である必要があります"):
            FinancialPlan(name="無効プラン", start_age=-1, end_age=30, annual_payment=100000)
    ...
    def test_end_age_less_than_start_age_raises_error(self):
        ...
        with pytest.raises(ValueError, match="終了年齢.*は開始年齢.*より大きい必要があります"):
            FinancialPlan(name="無効プラン", start_age=30, end_age=25, annual_payment=100000)

    def test_end_age_equal_to_start_age_raises_error(self):
        """終了年齢と開始年齢が同じでValueErrorが発生することを確認"""
        with pytest.raises(ValueError, match="終了年齢.*は開始年齢.*より大きい必要があります"):
            FinancialPlan(name="無効プラン", start_age=30, end_age=30, annual_payment=100000)
    ...
    def test_negative_annual_payment_raises_error(self):
        ...
        with pytest.raises(ValueError, match="年間支払額は0以上である必要があります"):
            FinancialPlan(name="無効プラン", start_age=30, end_age=60, annual_payment=-100000)

    def test_zero_start_age_is_valid(self):
        """開始年齢0歳は有効であることを確認"""
        plan = FinancialPlan(name="出生時プラン", start_age=0, end_age=18, annual_payment=50000)
...
class TestFinancialPlanProperties:
    ...
    def test_duration_years_short_plan(self):
        """短期プランの期間（年数）計算"""
        plan = FinancialPlan(name="短期プラン", start_age=25, end_age=30, annual_payment=50000)
    ...
    def test_total_payment_with_fractional_amount(self):
        """小数点付き年間支払額での総支払額計算"""
        plan = FinancialPlan(name="変額プラン", start_age=30, end_age=40, annual_payment=123456.78)
...
class TestFinancialPlanEdgeCases:
    ...
    def test_very_long_duration_plan(self):
        """非常に長期間のプラン"""
        plan = FinancialPlan(name="超長期プラン", start_age=0, end_age=100, annual_payment=50000)
        assert plan.duration_years == 100
        assert plan.total_payment == 5000000.0

    def test_very_high_annual_payment(self):
        """非常に高額な年間支払額"""
        plan = FinancialPlan(name="高額プラン", start_age=30, end_age=60, annual_payment=10000000)
        assert plan.annual_payment == 10000000
        assert plan.total_payment == 300000000.0

    def test_plan_with_fractional_payment(self):
        """小数点付き年間支払額のプラン"""
        plan = FinancialPlan(name="変額プラン", start_age=30, end_age=35, annual_payment=123456.789)
