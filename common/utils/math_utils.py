...
def calculate_annuity_present_value(payment: float, rate: float, periods: int) -> float:
    """年金現価計算（定額年金の現在価値）
...
def calculate_annuity_future_value(payment: float, rate: float, periods: int) -> float:
    """年金終価計算（定額年金の将来価値）
...
def calculate_irr(
    cash_flows: List[float], guess: float = 0.1, max_iterations: int = 100, tolerance: float = 1e-6
) -> Optional[float]:
